...
def build_dataset(set_options, common_options):
    ...
    # "included_frames": optional list like [1,2,3], for loading InMemory version.
    if type == "ABACUSDataset":
        assert "pbc" in set_options, "PBC must be provided in `data_option` when loading ABACUS dataset."
    ...
    if type == "ABACUSInMemoryDataset":
        assert "pbc" in set_options, "PBC must be provided in loading ABACUS dataset."
        AtomicDataOptions["pbc"] = set_options["pbc"]
    ...
    if type == "DefaultDataset":
        if "lcao_basis" in common_options:
            idp = OrbitalMapper(common_options["lcao_basis"])
        else:
            idp = None
